...
> **Next leaf to work: `3.a.iii.zi`** *(Phase 3 — Motion, Metrics & Charts, Animation System, Loading states — skeleton shimmer components. `3.a.iv` (Install Button) is now fully closed; `3.a.iii` is earlier in path order and was still open, so it's next per Section 1's "zi before zo; within a track before the next milestone" rule. `5.f.i.zi` — provision Supabase — remains gated behind Phases 1–4 per existing ordering.)*
