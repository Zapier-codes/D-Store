...
> **Next leaf to work: `0.h.ii.zo`** *(Geo-Regionalization Foundation — Region-filter helper in the dummy data-fetch layer (`0.a.ii.zo`) that narrows `apps` by the detected region — the seam future Play-Store-style regionalized queries plug into once Supabase (`5.f.i`) lands. Closes out `0.h` and Phase 0's UI revamp priority, see Section 2. `5.f.i.zi` — provision Supabase — resumes once Phase 0 is complete.)*
