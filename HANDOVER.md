...
> **Next leaf to work: `0.h.ii.zi`** *(Geo-Regionalization Foundation — Extend `lib/mock-data.ts` `App` shape with a dummy `available_regions: string[]` field; still 100% local dummy data, no live catalog calls. Part of Phase 0, UI revamp priority, see Section 2. `5.f.i.zi` — provision Supabase — resumes once Phase 0 is complete.)*
