...
> **Next leaf to work: `3.a.ii.zo`** *(Phase 3 — Motion, Metrics & Charts, Animation System, Micro-interactions — install-button progress animation. Note: this app has no real install button yet — that's a Phase 5/backend-connected concern — so this leaf will likely need a dummy/simulated progress state on a placeholder install action, or a scope split, before writing any animation code. `5.f.i.zi` — provision Supabase — remains gated behind Phases 1–4 per existing ordering.)*
